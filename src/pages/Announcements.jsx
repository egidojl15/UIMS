...
import { Calendar, FileText, Plus, Edit2, Trash2, X } from "lucide-react";
import { announcementsAPI } from "../services/api";

const Announcements = ({ editable = false }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(res.announcements ?? res.data ?? res ?? []);
    } catch (e) {
      console.error("load announcements", e);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = async () => {
    ...
    try {
      await announcementsAPI.update(
        editingItem.announcement_id || editingItem.id,
        {
          title: formData.title,
          content: formData.content,
        }
      ...
      console.error("update announcement", e);
    }
  };
      ...
      <section className="relative pt-32 pb-16 px-6 sm:px-8 lg:px-12">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Stay updated with the latest news and events in Upper Ichon
          </p>
          ...
          {loading ? (
            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-xl p-16 text-center border border-white/20 col-span-full">
              ...
              <p className="text-gray-500 max-w-md mx-auto">
                Check back soon for updates from the Upper Ichon community.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {items.map((a, i) => (
                ...
                >
                  ...
                  <div className="relative z-10">
                      ...
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                            <Calendar size={12} />
                            {a.posted_date || a.created_at
                              ? new Date(a.posted_date || a.created_at).toLocaleDateString("en-US", {
                                  month: "short",
                                  day: "numeric",
                                })
                              : "Recent"}
                      ...
                      {isEditable && (
                        ...
                        >
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDelete(a.announcement_id || a.id);
                            }}
                    ...
                    {a.content && (
                      <div className="mt-3 p-4 bg-gradient-to-r from-[#B3DEF8]/10 to-transparent rounded-xl border-l-4 border-gradient-to-b from-[#58A1D3] to-[#B3DEF8] group-hover:from-[#B3DEF8]/20 group-hover:to-cyan-50/50 transition-all duration-300">
        ...
        <Modal title={selectedItem.title} onClose={closeDetailModal}>
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-sm text-gray-600">
              <Calendar size={16} />
              <span>
                {selectedItem.posted_date || selectedItem.created_at
                  ? new Date(selectedItem.posted_date || selectedItem.created_at).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })
